...
public class TimeConverter {
	...
	public String convertStrToDateTime(final String time) {
		if (time != null && !"".equals(time)) {
			try {
				final int iYear = Integer.parseInt(time.substring(0, 4));
				final int iMonth = Integer.parseInt(time.substring(4, 6));
				final int iDate = Integer.parseInt(time.substring(6, 8));
				if (time.length() > 8) {
					final int iSecond = Integer.parseInt(time.substring(12, 17));
					final int iHours = iSecond / 3600;
					final int iMinutes = (iSecond % 3600) / 60;
					final int iSeconds = iSecond % 60;
					return String.format("%02d/%02d/%d %02d:%02d:%02d", iDate, iMonth, iYear, iHours, iMinutes, iSeconds);
				} else {
					// dd/MM/yyyy
					return String.format("%02d/%02d/%d", iDate, iMonth, iYear);
				}
	...
	public String convertStrToDtTranx(final String time) {
		if (time != null && !"".equals(time)) {
			...
			try {
				...
				return String.format("%02d/%02d/%d %02d:%02d:%02d", iDate, iMonth, iYear, iHours, iMinutes, iSecond);
			} catch (IndexOutOfBoundsException ex) {
	 ...
	 * 
	 * @param String
	 *            type: Neu FULL se bao gom HH24MISSSSS
	 * @return String
	 */
	public String getCurrentTime(final String type) {
		...
		if (type == "FULL") {
			...
			return String.format("%d%02d%02d", iYear, iMonth, iDate);
		}
